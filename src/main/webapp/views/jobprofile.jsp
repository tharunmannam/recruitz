...
<li><a class="active" href="/jobprofile">Job Profile</a></li>
    			 ...
    			 <c:if test="${(jobid == ssjobid) && (studemail == email) }">
					...
					<tbody>
						<tr >
							...
							<td><h7 class="mb-0">${job.jobRole}</h7></td>
							<td><h7 class="mb-0">${job.loc}</h7></td>
							
							<c:choose> 
							  <c:when test="${applied == 1}">
							  		<td><h7 class="mb-0">Applied</h7></td>
							  		<td><span class="btn btn-danger"><a href="/withdraw/${email}/${job.jobId}" style="color:white;">Withdraw</a></span></td>
							  </c:when>
							   <c:otherwise>
									<td><h7 >Not Applied</h7></td>  
									<td><button class="btn btn-secondary" disabled="">Withdraw</button></td>  	
							  </c:otherwise>
							</c:choose>
